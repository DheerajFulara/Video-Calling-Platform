// // // // // import React, { useState, useCallback, useEffect } from "react";
// // // // // import { useNavigate } from "react-router-dom";
// // // // // import { useSocket } from "./socketprovider";
// // // // // import axios from "axios";
// // // // // import { CopyToClipboard } from "react-copy-to-clipboard";
// // // // // import { Button } from "@mui/material";
// // // // // import { GoFile } from "react-icons/go";

// // // // // const inputstyle = {
// // // // //   border: "3px solid #B0DAFF",
// // // // //   width: "80%",
// // // // //   height: "35px",
// // // // //   fontSize: "20px",
// // // // // };

// // // // // const LobbyScreen = () => {
// // // // //   const [email, setEmail] = useState("");
// // // // //   const [room, setRoom] = useState("");
// // // // //   const [isLoading, setIsLoading] = useState(true);

// // // // //   const { socket, callEnded } = useSocket();
// // // // //   const navigate = useNavigate();

// // // // //   // to toggle between buttons
// // // // //   const [linktocall, setLinktocall] = useState("");
// // // // //   const [joinacall, setJoinacall] = useState("");
// // // // //   const [me, setMe] = useState("");

// // // // //   const handleSetCall = () => {
// // // // //     setLinktocall("1");
// // // // //   };

// // // // //   const handlejoincall = () => {
// // // // //     setJoinacall("1");
// // // // //   };

// // // // //   const handleback = () => {
// // // // //     setLinktocall("");
// // // // //     setJoinacall("");
// // // // //   };

// // // // //   // Check authentication and fetch user data
// // // // //   useEffect(() => {
// // // // //     const jwt = localStorage.getItem("access-token");
// // // // //     if (!jwt) {
// // // // //       window.location.href = "/login";
// // // // //       return;
// // // // //     }

// // // // //     const result = `${import.meta.env.VITE_API_URL}/auth`;

// // // // //     axios
// // // // //       .get(result, {
// // // // //         headers: { authorization: `Bearer: ${jwt}` },
// // // // //       })
// // // // //       .then((res) => {
// // // // //         setEmail(res.data.Email);
// // // // //         setMe(res.data.Category);
// // // // //         setIsLoading(false); // Auth successful, show UI
// // // // //       })
// // // // //       .catch((err) => {
// // // // //         console.log("error here is --> ", JSON.stringify(err));
// // // // //         localStorage.removeItem("access-token");
// // // // //         window.location.href = "/login";
// // // // //       });
// // // // //   }, []); // Run only once on mount

// // // // //   const handleSubmitForm = useCallback(
// // // // //     (e) => {
// // // // //       console.log("JOINED");
// // // // //       e.preventDefault();
// // // // //       socket.emit("room:join", { email, room });
// // // // //     },
// // // // //     [email, room, socket],
// // // // //   );

// // // // //   const handleJoinRoom = useCallback(
// // // // //     (data) => {
// // // // //       const { email, room } = data;
// // // // //       console.log("email", email);
// // // // //       navigate(`/room/${room}`);
// // // // //     },
// // // // //     [navigate],
// // // // //   );

// // // // //   useEffect(() => {
// // // // //     socket.on("me", (id) => {
// // // // //       console.log(id);
// // // // //     });
// // // // //     socket.on("room:join", handleJoinRoom);
// // // // //     return () => {
// // // // //       socket.off("room:join", handleJoinRoom);
// // // // //     };
// // // // //   }, [socket, handleJoinRoom]);

// // // // //   // Don't render UI until authentication check is complete
// // // // //   if (isLoading) {
// // // // //     return (
// // // // //       <div className="App">
// // // // //         <div style={{ textAlign: "center", marginTop: "50%" }}>
// // // // //           <h2>Loading...</h2>
// // // // //         </div>
// // // // //       </div>
// // // // //     );
// // // // //   }

// // // // //   return (
// // // // //     <div className="App">
// // // // //       <div>
// // // // //         <h1
// // // // //           style={{
// // // // //             marginTop: "22%",
// // // // //             textAlign: "center",
// // // // //             fontFamily: "Caveat",
// // // // //           }}
// // // // //         >
// // // // //           Make a video call{" "}
// // // // //         </h1>
// // // // //         <form onSubmit={handleSubmitForm}>
// // // // //           <div align="center">
// // // // //             <h3 style={{ textAlign: "center", marginTop: "70px" }}>
// // // // //               {" "}
// // // // //               Email ID : {email}{" "}
// // // // //             </h3>

// // // // //             {!linktocall && !joinacall && (
// // // // //               <div align="center">
// // // // //                 <Button
// // // // //                   variant="contained"
// // // // //                   style={{
// // // // //                     margin: "auto",
// // // // //                     width: "80%",
// // // // //                     backgroundColor: "#DAF5FF",
// // // // //                     color: "black",
// // // // //                   }}
// // // // //                   onClick={() => {
// // // // //                     handleSetCall();
// // // // //                   }}
// // // // //                 >
// // // // //                   Create a link for calling{" "}
// // // // //                 </Button>
// // // // //               </div>
// // // // //             )}

// // // // //             {linktocall && <h3 style={{ marginTop: "20px" }}>Link : {me} </h3>}
// // // // //             {linktocall && (
// // // // //               <>
// // // // //                 <CopyToClipboard text={me}>
// // // // //                   <Button
// // // // //                     variant="contained"
// // // // //                     style={{
// // // // //                       margin: "auto",
// // // // //                       width: "50%",
// // // // //                       backgroundColor: "#DAF5FF",
// // // // //                       color: "black",
// // // // //                     }}
// // // // //                     startIcon={<GoFile fontSize="small" />}
// // // // //                   >
// // // // //                     Copy Link
// // // // //                   </Button>
// // // // //                 </CopyToClipboard>
// // // // //               </>
// // // // //             )}
// // // // //           </div>
// // // // //           <br />
// // // // //           {!joinacall && !linktocall && (
// // // // //             <div align="center">
// // // // //               <Button
// // // // //                 variant="contained"
// // // // //                 style={{
// // // // //                   margin: "auto",
// // // // //                   width: "80%",
// // // // //                   backgroundColor: "#DAF5FF",
// // // // //                   color: "black",
// // // // //                 }}
// // // // //                 onClick={() => {
// // // // //                   handlejoincall();
// // // // //                 }}
// // // // //               >
// // // // //                 {" "}
// // // // //                 Join a call{" "}
// // // // //               </Button>
// // // // //             </div>
// // // // //           )}

// // // // //           {(joinacall || linktocall) && (
// // // // //             <>
// // // // //               <br />
// // // // //               <div align="center">
// // // // //                 <h3 style={{ marginTop: "20px" }}> Paste the Link here </h3>
// // // // //                 <input
// // // // //                   style={inputstyle}
// // // // //                   type="text"
// // // // //                   id="room"
// // // // //                   value={room}
// // // // //                   onChange={(e) => setRoom(e.target.value)}
// // // // //                 />
// // // // //                 <br />
// // // // //                 <button
// // // // //                   style={{
// // // // //                     marginTop: "20px",
// // // // //                     width: "80%",
// // // // //                     height: "40px",
// // // // //                     backgroundColor: "#DAF5FF",
// // // // //                     borderRadius: "10px",
// // // // //                     fontSize: "20px",
// // // // //                     color: "black",
// // // // //                   }}
// // // // //                 >
// // // // //                   Join
// // // // //                 </button>
// // // // //               </div>
// // // // //             </>
// // // // //           )}
// // // // //         </form>
// // // // //       </div>

// // // // //       {(joinacall || linktocall) && (
// // // // //         <Button
// // // // //           variant="contained"
// // // // //           style={{
// // // // //             margin: "auto",
// // // // //             width: "80%",
// // // // //             backgroundColor: "#DAF5FF",
// // // // //             color: "black",
// // // // //           }}
// // // // //           onClick={() => {
// // // // //             handleback();
// // // // //           }}
// // // // //         >
// // // // //           {" "}
// // // // //           Go Back{" "}
// // // // //         </Button>
// // // // //       )}

// // // // //       <Button
// // // // //         variant="contained"
// // // // //         style={{
// // // // //           margin: "auto",
// // // // //           width: "80%",
// // // // //           backgroundColor: "#DAF5FF",
// // // // //           color: "black",
// // // // //         }}
// // // // //         onClick={() => {
// // // // //           navigate("/home");
// // // // //         }}
// // // // //       >
// // // // //         {" "}
// // // // //         Go to home{" "}
// // // // //       </Button>
// // // // //     </div>
// // // // //   );
// // // // // };

// // // // // export default LobbyScreen;

// // // // import React, { useState, useCallback, useEffect } from "react";
// // // // import { useNavigate } from "react-router-dom";
// // // // import { useSocket } from "./socketprovider";
// // // // import axios from "axios";
// // // // import { CopyToClipboard } from 'react-copy-to-clipboard';
// // // // import { Button } from '@mui/material';
// // // // import { GoFile } from "react-icons/go";
// // // // // import { IoArrowBackCircleOutline } from "react-icons/io5";

// // // // const inputstyle = {
// // // //   border: "3px solid #B0DAFF",
// // // //   width: "80%",
// // // //   height: "35px",
// // // //   fontSize: "20px"
// // // // }

// // // // const LobbyScreen = () => {
// // // //   const [email, setEmail] = useState("");
// // // //   const [room, setRoom] = useState("");

// // // //   const { socket, callEnded } = useSocket()
// // // //   const navigate = useNavigate();

// // // //   // to toggle between buttons
// // // //   const [linktocall, setLinktocall] = useState("");
// // // //   const [joinacall, setJoinacall] = useState("");
// // // //   const [me, setMe] = useState("")

// // // //   const handleSetCall = () => {
// // // //     setLinktocall("1");
// // // //   };

// // // //   const handlejoincall = () => {
// // // //     setJoinacall("1");
// // // //   };

// // // //   const handleback = () => {
// // // //     // navigate("/lobby")
// // // //     setLinktocall("")
// // // //     setJoinacall("")
// // // //   };

// // // //   // jwt code for getting email - wrapped in useEffect to prevent infinite renders
// // // //   useEffect(() => {
// // // //     const jwt = localStorage.getItem("access-token");
// // // //     if (!jwt) {
// // // //       window.location.href = "/login";
// // // //       return;
// // // //     }

// // // //     const result = `${import.meta.env.VITE_API_URL}/auth`;

// // // //     axios
// // // //       .get(result, {
// // // //         headers: { authorization: `Bearer: ${jwt}` }
// // // //       })
// // // //       .then(res => {
// // // //         setEmail(res.data.Email);
// // // //         setMe(res.data.Category);
// // // //       })
// // // //       .catch(err => {
// // // //         console.log("error here is -->  ", JSON.stringify(err));
// // // //         localStorage.removeItem("access-token");
// // // //         window.location.href = "/login";
// // // //       });
// // // //   }, []); // Run only once on mount

// // // //   const handleSubmitForm = useCallback(
// // // //     (e) => {
// // // //       console.log("JOINED")
// // // //       e.preventDefault();
// // // //       socket.emit("room:join", { email, room });

// // // //     },
// // // //     [email, room, socket]
// // // //   );

// // // //   const handleJoinRoom = useCallback(
// // // //     (data) => {
// // // //       const { email, room } = data;
// // // //       console.log("email", email)
// // // //       navigate(`/room/${room}`);
// // // //     },
// // // //     [navigate]
// // // //   );

// // // //   useEffect(() => {
// // // //     socket.on("me", (id) => {
// // // //       console.log(id)
// // // //     })
// // // //     socket.on("room:join", handleJoinRoom);
// // // //     return () => {
// // // //       socket.off("room:join", handleJoinRoom);
// // // //     };
// // // //   }, [socket, handleJoinRoom]);

// // // //   return (
// // // //     <div className="App">
// // // //       <div>
// // // //         <h1 style={{ marginTop: "22%", textAlign: "center", fontFamily: 'Caveat' }}>Make a video call </h1>
// // // //         <form onSubmit={handleSubmitForm}>
// // // //           <div align="center">
// // // //             <h3 style={{ textAlign: "center", marginTop: "70px" }}> Email ID : {email} </h3>

// // // //             {!linktocall && !joinacall && (
// // // //               <div align="center">
// // // //                 <Button variant='contained' style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black" }} onClick={() => { handleSetCall() }}>Create a link for calling </Button>
// // // //               </div>
// // // //             )}

// // // //             {linktocall && (
// // // //               <h3 style={{ marginTop: "20px" }}> Link : {me} </h3>
// // // //             )}
// // // //             {linktocall && (
// // // //               <>
// // // //                 <CopyToClipboard text={me} >
// // // //                   <Button variant="contained" style={{ margin: "auto", width: "50%", backgroundColor: "#DAF5FF", color: "black" }} startIcon={<GoFile fontSize="small" />}>
// // // //                     Copy Link
// // // //                   </Button>
// // // //                 </CopyToClipboard>
// // // //               </>
// // // //             )}
// // // //           </div>
// // // //           <br />
// // // //           {!joinacall && !linktocall && (
// // // //             <div align="center">
// // // //               <Button variant='contained' style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black" }} onClick={() => { handlejoincall() }}> Join a call  </Button>
// // // //             </div>
// // // //           )}

// // // //           {(joinacall || linktocall) && (<>
// // // //             <br />
// // // //             <div align="center">
// // // //               <h3 style={{ marginTop: "20px" }}> Paste the Link here </h3>
// // // //               <input
// // // //                 style={inputstyle}
// // // //                 type="text"
// // // //                 id="room"
// // // //                 value={room}
// // // //                 onChange={(e) => setRoom(e.target.value)}
// // // //               />
// // // //               <br />
// // // //               <button
// // // //                 style={{ marginTop: "20px", width: "80%", height: "40px", backgroundColor: "#DAF5FF", borderRadius: '10px', fontSize: '20px', color: "black" }}
// // // //               >Join</button>
// // // //             </div>
// // // //           </>)
// // // //           }

// // // //         </form>
// // // //       </div>

// // // //       {(joinacall || linktocall) && (
// // // //         <Button variant='contained' style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black" }} onClick={() => { handleback() }}> Go Back  </Button>
// // // //       )}

// // // //       <Button variant='contained' style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black" }} onClick={() => { navigate("/home") }}> Go to home  </Button>

// // // //     </div>
// // // //   );
// // // // };

// // // // export default LobbyScreen;

// // // import React, { useState, useCallback, useEffect } from "react";
// // // import { useNavigate } from "react-router-dom";
// // // import { useSocket } from "./socketprovider";
// // // import axios from "axios";
// // // import { CopyToClipboard } from 'react-copy-to-clipboard';
// // // import { Button } from '@mui/material';
// // // import { GoFile } from "react-icons/go";
// // // import { v4 as uuidv4 } from "uuid";

// // // const inputstyle = {
// // //   border: "3px solid #B0DAFF",
// // //   width: "80%",
// // //   height: "35px",
// // //   fontSize: "20px"
// // // };

// // // const LobbyScreen = () => {
// // //   const [email, setEmail] = useState("");
// // //   const [room, setRoom] = useState("");
// // //   const [me, setMe] = useState("");
// // //   const [isLoading, setIsLoading] = useState(true); // Loading state

// // //   const [linktocall, setLinktocall] = useState("");
// // //   const [joinacall, setJoinacall] = useState("");

// // //   const { socket } = useSocket();
// // //   const navigate = useNavigate();

// // //   // Auth check
// // //   useEffect(() => {
// // //     const jwt = localStorage.getItem("access-token");
// // //     if (!jwt) {
// // //       window.location.href = "/login";
// // //       return;
// // //     }

// // //     const result = `${import.meta.env.VITE_API_URL}/auth`;

// // //     axios.get(result, { headers: { authorization: `Bearer: ${jwt}` } })
// // //       .then(res => {
// // //         setEmail(res.data.Email);
// // //         setMe(res.data.Category); // Can use as user identifier
// // //         setIsLoading(false); // Auth success, show UI
// // //       })
// // //       .catch(err => {
// // //         console.log("Auth error --> ", JSON.stringify(err));
// // //         localStorage.removeItem("access-token");
// // //         window.location.href = "/login";
// // //       });
// // //   }, []);

// // //   // Generate a room link
// // //   const handleSetCall = () => {
// // //     const roomId = uuidv4();
// // //     setRoom(roomId);
// // //     setLinktocall("1");
// // //   };

// // //   const handlejoincall = () => {
// // //     setJoinacall("1");
// // //   };

// // //   const handleback = () => {
// // //     setLinktocall("");
// // //     setJoinacall("");
// // //     setRoom("");
// // //   };

// // //   // Join room
// // //   const handleSubmitForm = useCallback(
// // //     (e) => {
// // //       e.preventDefault();
// // //       if (!room) return alert("Please enter a room link to join!");
// // //       socket.emit("room:join", { email, room });
// // //     },
// // //     [email, room, socket]
// // //   );

// // //   const handleJoinRoom = useCallback(
// // //     (data) => {
// // //       const { room } = data;
// // //       navigate(`/room/${room}`);
// // //     },
// // //     [navigate]
// // //   );

// // //   useEffect(() => {
// // //     socket.on("room:join", handleJoinRoom);
// // //     return () => {
// // //       socket.off("room:join", handleJoinRoom);
// // //     };
// // //   }, [socket, handleJoinRoom]);

// // //   // Show loading UI while checking auth
// // //   if (isLoading) {
// // //     return (
// // //       <div className="App">
// // //         <div style={{ textAlign: "center", marginTop: "50%" }}>
// // //           <h2>Loading...</h2>
// // //         </div>
// // //       </div>
// // //     );
// // //   }

// // //   return (
// // //     <div className="App">
// // //       <div>
// // //         <h1 style={{ marginTop: "22%", textAlign: "center", fontFamily: 'Caveat' }}>
// // //           Make a video call
// // //         </h1>
// // //         <form onSubmit={handleSubmitForm}>
// // //           <div align="center">
// // //             <h3 style={{ textAlign: "center", marginTop: "70px" }}> Email ID : {email} </h3>

// // //             {!linktocall && !joinacall && (
// // //               <>
// // //                 <Button
// // //                   variant='contained'
// // //                   style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black", marginBottom: '15px' }}
// // //                   onClick={handleSetCall}
// // //                 >
// // //                   Create a link for calling
// // //                 </Button>

// // //                 <Button
// // //                   variant='contained'
// // //                   style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black" }}
// // //                   onClick={handlejoincall}
// // //                 >
// // //                   Join a call
// // //                 </Button>
// // //               </>
// // //             )}

// // //             {linktocall && (
// // //               <>
// // //                 <h3 style={{ marginTop: "20px" }}> Link : {room} </h3>
// // //                 <CopyToClipboard text={room}>
// // //                   <Button
// // //                     variant="contained"
// // //                     style={{ margin: "auto", width: "50%", backgroundColor: "#DAF5FF", color: "black" }}
// // //                     startIcon={<GoFile fontSize="small" />}
// // //                   >
// // //                     Copy Link
// // //                   </Button>
// // //                 </CopyToClipboard>
// // //               </>
// // //             )}

// // //             {(joinacall || linktocall) && (
// // //               <>
// // //                 <br />
// // //                 <div align="center">
// // //                   <h3 style={{ marginTop: "20px" }}> Paste the Link here </h3>
// // //                   <input
// // //                     style={inputstyle}
// // //                     type="text"
// // //                     value={room}
// // //                     onChange={(e) => setRoom(e.target.value)}
// // //                   />
// // //                   <br />
// // //                   <button
// // //                     type="submit"
// // //                     style={{
// // //                       marginTop: "20px",
// // //                       width: "80%",
// // //                       height: "40px",
// // //                       backgroundColor: "#DAF5FF",
// // //                       borderRadius: '10px',
// // //                       fontSize: '20px',
// // //                       color: "black"
// // //                     }}
// // //                   >
// // //                     Join
// // //                   </button>
// // //                 </div>
// // //               </>
// // //             )}
// // //           </div>
// // //         </form>
// // //       </div>

// // //       {(joinacall || linktocall) && (
// // //         <Button
// // //           variant='contained'
// // //           style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black", marginTop: "20px" }}
// // //           onClick={handleback}
// // //         >
// // //           Go Back
// // //         </Button>
// // //       )}

// // //       <Button
// // //         variant='contained'
// // //         style={{ margin: "auto", width: "80%", backgroundColor: "#DAF5FF", color: "black", marginTop: "20px" }}
// // //         onClick={() => navigate("/home")}
// // //       >
// // //         Go to home
// // //       </Button>
// // //     </div>
// // //   );
// // // };

// // // export default LobbyScreen;

// // import React, { useState, useCallback, useEffect } from "react";
// // import { useNavigate } from "react-router-dom";
// // import { useSocket } from "./socketprovider";
// // import axios from "axios";
// // import { CopyToClipboard } from "react-copy-to-clipboard";
// // import { Button } from "@mui/material";
// // import { GoFile } from "react-icons/go";
// // // import { IoArrowBackCircleOutline } from "react-icons/io5";

// // const inputstyle = {
// //   border: "3px solid #B0DAFF",
// //   width: "80%",
// //   height: "35px",
// //   fontSize: "20px",
// // };

// // export const LobbyScreen = () => {
// //   const [email, setEmail] = useState("");
// //   const [room, setRoom] = useState("");

// //   const { socket, callEnded } = useSocket();
// //   const navigate = useNavigate();

// //   // to toggle between buttons
// //   const [linktocall, setLinktocall] = useState("");
// //   const [joinacall, setJoinacall] = useState("");
// //   const [me, setMe] = useState("");

// //   const handleSetCall = () => {
// //     setLinktocall("1");
// //   };

// //   const handlejoincall = () => {
// //     setJoinacall("1");
// //   };

// //   const handleback = () => {
// //     // navigate("/lobby")
// //     setLinktocall("");
// //     setJoinacall("");
// //   };

// //   // jwt code for getting email
// //   // ******
// //   const jwt = localStorage.getItem("access-token");
// //   if (!jwt) {
// //     window.location.href = "/login";
// //   } else {
// //     let st1 = process.env.VITE_API_URL;
// //     let st2 = "/auth";
// //     let result = st1.concat(st2);
// //     // console.log(result);
// //     axios
// //       .get(result, {
// //         headers: { authorization: `Bearer: ${jwt}` },
// //       })
// //       .then((res) => {
// //         // console.log("VERIFIED USER");
// //         // console.log(res.data);
// //         setEmail(res.data.Email);
// //         setMe(res.data.Category);
// //       })
// //       .catch((err) => {
// //         console.log("error here is -->  ", JSON.stringify(err));
// //         localStorage.removeItem("access-token");
// //         window.location.href = "/login";
// //       });
// //   }

// //   const handleSubmitForm = useCallback(
// //     (e) => {
// //       console.log("JOINED");
// //       e.preventDefault();
// //       socket.emit("room:join", { email, room });
// //     },
// //     [email, room, socket],
// //   );

// //   const handleJoinRoom = useCallback(
// //     (data) => {
// //       const { email, room } = data;
// //       console.log("email", email);
// //       navigate(`/room/${room}`);
// //     },
// //     [navigate],
// //   );

// //   useEffect(() => {
// //     socket.on("me", (id) => {
// //       console.log(id);
// //     });
// //     socket.on("room:join", handleJoinRoom);
// //     return () => {
// //       socket.off("room:join", handleJoinRoom);
// //     };
// //   }, [socket, handleJoinRoom]);

// //   return (
// //     <div className="App">
// //       <div>
// //         <h1
// //           style={{
// //             marginTop: "22%",
// //             textAlign: "center",
// //             fontFamily: "Caveat",
// //           }}
// //         >
// //           Make a video call{" "}
// //         </h1>
// //         <form onSubmit={handleSubmitForm}>
// //           <div align="center">
// //             <h3 style={{ textAlign: "center", marginTop: "70px" }}>
// //               {" "}
// //               Email ID : {email}{" "}
// //             </h3>

// //             {!linktocall && !joinacall && (
// //               <div align="center">
// //                 <Button
// //                   variant="contained"
// //                   style={{
// //                     margin: "auto",
// //                     width: "80%",
// //                     backgroundColor: "#DAF5FF",
// //                     color: "black",
// //                   }}
// //                   onClick={() => {
// //                     handleSetCall();
// //                   }}
// //                 >
// //                   Create a link for calling{" "}
// //                 </Button>
// //               </div>
// //             )}

// //             {linktocall && <h3 style={{ marginTop: "20px" }}> Link : {me} </h3>}
// //             {linktocall && (
// //               <>
// //                 <CopyToClipboard text={me}>
// //                   <Button
// //                     variant="contained"
// //                     style={{
// //                       margin: "auto",
// //                       width: "50%",
// //                       backgroundColor: "#DAF5FF",
// //                       color: "black",
// //                     }}
// //                     startIcon={<GoFile fontSize="small" />}
// //                   >
// //                     Copy Link
// //                   </Button>
// //                 </CopyToClipboard>
// //               </>
// //             )}
// //           </div>
// //           <br />
// //           {!joinacall && !linktocall && (
// //             <div align="center">
// //               <Button
// //                 variant="contained"
// //                 style={{
// //                   margin: "auto",
// //                   width: "80%",
// //                   backgroundColor: "#DAF5FF",
// //                   color: "black",
// //                 }}
// //                 onClick={() => {
// //                   handlejoincall();
// //                 }}
// //               >
// //                 {" "}
// //                 Join a call{" "}
// //               </Button>
// //             </div>
// //           )}

// //           {(joinacall || linktocall) && (
// //             <>
// //               <br />
// //               <div align="center">
// //                 <h3 style={{ marginTop: "20px" }}> Paste the Link here </h3>
// //                 <input
// //                   style={inputstyle}
// //                   type="text"
// //                   id="room"
// //                   value={room}
// //                   onChange={(e) => setRoom(e.target.value)}
// //                 />
// //                 <br />
// //                 <button
// //                   style={{
// //                     marginTop: "20px",
// //                     width: "80%",
// //                     height: "40px",
// //                     backgroundColor: "#DAF5FF",
// //                     borderRadius: "10px",
// //                     fontSize: "20px",
// //                     color: "black",
// //                   }}
// //                 >
// //                   Join
// //                 </button>
// //               </div>
// //             </>
// //           )}
// //         </form>
// //       </div>

// //       {(joinacall || linktocall) && (
// //         <Button
// //           variant="contained"
// //           style={{
// //             margin: "auto",
// //             width: "80%",
// //             backgroundColor: "#DAF5FF",
// //             color: "black",
// //           }}
// //           onClick={() => {
// //             handleback();
// //           }}
// //         >
// //           {" "}
// //           Go Back{" "}
// //         </Button>
// //       )}

// //       <Button
// //         variant="contained"
// //         style={{
// //           margin: "auto",
// //           width: "80%",
// //           backgroundColor: "#DAF5FF",
// //           color: "black",
// //         }}
// //         onClick={() => {
// //           navigate("/home");
// //         }}
// //       >
// //         {" "}
// //         Go to home{" "}
// //       </Button>
// //     </div>
// //   );
// // };

// import React, { useState, useCallback, useEffect } from "react";
// import { useNavigate } from "react-router-dom";
// import { useSocket } from "./socketprovider";
// import axios from "axios";
// import { CopyToClipboard } from "react-copy-to-clipboard";
// import { Button } from "@mui/material";
// import { GoFile } from "react-icons/go";

// const inputstyle={
//   border: "3px solid #B0DAFF",
//   width: "80%",
//   height: "35px",
//   fontSize: "20px"
// }

// const LobbyScreen = () => {

//   const [email, setEmail] = useState("");
//     const [room, setRoom] = useState("");

//     const {socket , callEnded} = useSocket()
//     const navigate = useNavigate();

//     // to toggle between buttons
//     const [linktocall, setLinktocall] = useState("");
//     const [joinacall, setJoinacall] = useState("");
//     const [me , setMe] = useState("")

//     const handleSetCall = () => {
//     setLinktocall("1");
//   };

//   const handlejoincall = () => {
//     setJoinacall("1");
//   };

//   const handleback = () => {
//     // navigate("/lobby")
//     setLinktocall("")
//     setJoinacall("")
//   };

//   // const jwt = localStorage.getItem("access-token");
//   //   if (!jwt) {
//   //     // (window.location.href = "/login")
//   //   } else {
//   //     let st1 = process.env.VITE_API_URL;
//   //     let st2 = "/auth";
//   //     let result = st1.concat(st2);
//   //     // console.log(result);
//   //     axios
//   //         .get(result , {
//   //         headers: { authorization: `Bearer: ${jwt}` }
//   //       })
//   //       .then(res => {
//   //         // console.log("VERIFIED USER");
//   //         // console.log(res.data);
//   //         setEmail(res.data.Email);
//   //         setMe(res.data.Category);
//   //       })
//   //       .catch(err => {
//   //         console.log("error here is -->  ", JSON.stringify(err));
//   //         localStorage.removeItem("access-token");
//   //         (window.location.href = "/login")
//   //       });
//   //   }

//     // const handleSubmitForm = useCallback(
//     //   (e) => {
//     //     console.log("JOINED")
//     //     e.preventDefault();
//     //     socket.emit("room:join", { email, room });

//     //   },
//     //   [email, room, socket]
//     // );

//     // const handleJoinRoom = useCallback(
//     //   (data) => {
//     //     const { email, room } = data;
//     //     console.log("email",email)
//     //     navigate(`/room/${room}`);
//     //   },
//     //   [navigate]
//     // );

//   return (
//     <div>
//       <h1> lobby page</h1>
//     </div>
//   );
// };

// export default LobbyScreen;

import React, { useState, useCallback, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { useSocket } from "./socketprovider";
import axios from "axios";
import { CopyToClipboard } from "react-copy-to-clipboard";
import { Button } from "@mui/material";
import { GoFile } from "react-icons/go";
// import { IoArrowBackCircleOutline } from "react-icons/io5";

const inputstyle = {
  border: "3px solid #B0DAFF",
  width: "80%",
  height: "35px",
  fontSize: "20px",
};

const LobbyScreen = () => {
  const [email, setEmail] = useState("");
  const [room, setRoom] = useState("");
  const [isLoading, setIsLoading] = useState(true);

  const socket = useSocket();
  const navigate = useNavigate();

  // to toggle between buttons
  const [linktocall, setLinktocall] = useState("");
  const [joinacall, setJoinacall] = useState("");
  const [me, setMe] = useState("");

  const handleSetCall = () => {
    setLinktocall("1");
  };

  const handlejoincall = () => {
    setJoinacall("1");
  };

  const handleback = () => {
    // navigate("/lobby")
    setLinktocall("");
    setJoinacall("");
  };

  // jwt code for getting email
  // ******
  //

  useEffect(() => {
    const jwt = localStorage.getItem("access-token");

    if (!jwt) {
      window.location.href = "/login";
      return;
    }

    const apiUrl = import.meta.env.VITE_API_URL;
    const result = `${apiUrl}/auth`;

    axios
      .get(result, {
        headers: {
          authorization: `Bearer: ${jwt}`,
        },
      })
      .then((res) => {
        setEmail(res.data.Email);
        setMe(res.data.Category);
        setIsLoading(false);
      })
      .catch((err) => {
        console.log("error here is -->  ", JSON.stringify(err));
        localStorage.removeItem("access-token");
        window.location.href = "/login";
      });
  }, []);

  const handleSubmitForm = useCallback(
    (e) => {
      console.log("JOINED");
      e.preventDefault();
      socket.emit("room:join", { email, room });
    },
    [email, room, socket],
  );

  const handleJoinRoom = useCallback(
    (data) => {
      const { email, room } = data;
      console.log("email", email);
      navigate(`/room/${room}`);
    },
    [navigate],
  );

  useEffect(() => {
    if (!socket) return;

    socket.on("me", (id) => {
      console.log(id);
    });
    socket.on("room:join", handleJoinRoom);
    return () => {
      socket.off("room:join", handleJoinRoom);
    };
  }, [socket, handleJoinRoom]);

  // Don't render UI until authentication check is complete
  if (isLoading) {
    return (
      <div className="App">
        <div style={{ textAlign: "center", marginTop: "50%" }}>
          <h2>Loading...</h2>
        </div>
      </div>
    );
  }

  return (
    <div className="App">
      <div>
        <h1
          style={{
            marginTop: "22%",
            textAlign: "center",
            fontFamily: "Caveat",
          }}
        >
          Make a video call{" "}
        </h1>
        <form onSubmit={handleSubmitForm}>
          <div align="center">
            <h3 style={{ textAlign: "center", marginTop: "70px" }}>
              {" "}
              Email ID : {email}{" "}
            </h3>

            {!linktocall && !joinacall && (
              <div align="center">
                <Button
                  variant="contained"
                  style={{
                    margin: "auto",
                    width: "80%",
                    backgroundColor: "#DAF5FF",
                    color: "black",
                  }}
                  onClick={() => {
                    handleSetCall();
                  }}
                >
                  Create a link for calling{" "}
                </Button>
              </div>
            )}

            {linktocall && <h3 style={{ marginTop: "20px" }}> Link : {me} </h3>}
            {linktocall && (
              <>
                <CopyToClipboard text={me}>
                  <Button
                    variant="contained"
                    style={{
                      margin: "auto",
                      width: "50%",
                      backgroundColor: "#DAF5FF",
                      color: "black",
                    }}
                    startIcon={<GoFile fontSize="small" />}
                  >
                    Copy Link
                  </Button>
                </CopyToClipboard>
              </>
            )}
          </div>
          <br />
          {!joinacall && !linktocall && (
            <div align="center">
              <Button
                variant="contained"
                style={{
                  margin: "auto",
                  width: "80%",
                  backgroundColor: "#DAF5FF",
                  color: "black",
                }}
                onClick={() => {
                  handlejoincall();
                }}
              >
                {" "}
                Join a call{" "}
              </Button>
            </div>
          )}

          {(joinacall || linktocall) && (
            <>
              <br />
              <div align="center">
                <h3 style={{ marginTop: "20px" }}> Paste the Link here </h3>
                <input
                  style={inputstyle}
                  type="text"
                  id="room"
                  value={room}
                  onChange={(e) => setRoom(e.target.value)}
                />
                <br />
                <button
                  style={{
                    marginTop: "20px",
                    width: "80%",
                    height: "40px",
                    backgroundColor: "#DAF5FF",
                    borderRadius: "10px",
                    fontSize: "20px",
                    color: "black",
                  }}
                >
                  Join
                </button>
              </div>
            </>
          )}
        </form>
      </div>

      {(joinacall || linktocall) && (
        <Button
          variant="contained"
          style={{
            margin: "auto",
            width: "80%",
            backgroundColor: "#DAF5FF",
            color: "black",
          }}
          onClick={() => {
            handleback();
          }}
        >
          {" "}
          Go Back{" "}
        </Button>
      )}

      <Button
        variant="contained"
        style={{
          margin: "auto",
          width: "80%",
          backgroundColor: "#DAF5FF",
          color: "black",
        }}
        onClick={() => {
          navigate("/home");
        }}
      >
        {" "}
        Go to home{" "}
      </Button>
    </div>
  );
};

export default LobbyScreen;
